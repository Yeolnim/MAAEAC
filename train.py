...
from model.maaeac import MAAEAC
...
def main(config):
    ...
    datasets = {
        ...
        '2015': "atepc_datasets/2015",
        '2016': "atepc_datasets/2016",
    }
    pretrained_bert_models = {
        ...
        # for loading BERT-base
        # 'laptop': "bert/bert-base-uncased",
        # 'restaurant': "bert/bert-base-uncased",
        # '2015': "bert/bert-base-uncased",
        # '2016': "bert/bert-base-uncased",

        # for loading domain-adapted BERT
        'laptop': "bert/laptops_1mio_ep30",
        'restaurant': "bert/restaurants_10mio_ep3",
        '2015': "bert/restaurants_10mio_ep3",
        '2016': "bert/restaurants_10mio_ep3",

        # 'laptop': "bert/laptops_and_restaurants_2mio_ep15",
        # 'restaurant': "bert/laptops_and_restaurants_2mio_ep15",
    }
    ...
    if args.dataset in {'camera', 'car', 'phone', 'notebook'}:
        ...
        model = MAAEAC(bert_base_model, args=args)
    else:
        model = MAAEAC(bert_base_model, args=args)
    ...
    all_distance_to_aspect = torch.tensor([f.distance_to_aspect for f in eval_features], dtype=torch.long)
    all_aspect_indices = torch.tensor([f.aspect_indices for f in eval_features], dtype=torch.long)
    eval_data = TensorDataset(all_spc_input_ids, all_input_mask, all_segment_ids, all_label_ids,
                              all_polarities, all_valid_ids, all_lmask_ids, all_distance_to_aspect, all_aspect_indices)
    ...
    def evaluate(eval_ATE=True, eval_APC=True):
        ...
        for input_ids_spc, input_mask, segment_ids, label_ids, polarities, valid_ids, l_mask, distance_to_aspect, aspect_indices in eval_dataloader:#jy
            ...
            distance_to_aspect = distance_to_aspect.to(device)
            aspect_indices = aspect_indices.to(device)

            with torch.no_grad():
                ate_logits, apc_logits = model(input_ids_spc, segment_ids, input_mask,
                                               valid_ids=valid_ids, polarities=polarities,attention_mask_label=l_mask, distance_to_aspect=distance_to_aspect, aspect_indices=aspect_indices)#jy
            if eval_APC:
    ...
    def train():
        ...
        all_distance_to_aspect = torch.tensor([f.distance_to_aspect for f in train_features], dtype=torch.long)
        all_aspect_indices = torch.tensor([f.aspect_indices for f in train_features], dtype=torch.long)

        train_data = TensorDataset(all_spc_input_ids, all_input_mask, all_segment_ids,
                                   all_label_ids, all_polarities, all_valid_ids, all_lmask_ids, all_distance_to_aspect, all_aspect_indices)#jy
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                input_ids_spc, input_mask, segment_ids, label_ids, polarities, valid_ids, l_mask, distance_to_aspect, aspect_indices = batch
                loss_ate, loss_apc = model(input_ids_spc, segment_ids, input_mask, label_ids, polarities, valid_ids,
                                           l_mask, distance_to_aspect, aspect_indices)#jy
...
def parse_experiments(path):
    ...
    for id, config in json_config.items():
        ...
        parser.add_argument("--num_attention_heads", default=24, type=int)
